...
def main():
    ...
    for key_type in SUPPORTED_EC_KEY_TYPES:
        ...
        log.info("##############################################################")
        log.info("#  Testing ECC CSR genration - %s ###", key_type)
        log.info("##############################################################")
        ...
        run("%s ecparam --provider %s --provider default -name %s:0xEF000002 -genkey -out %s" %(openssl_bin, provider, key_type, ref_ec_key_0xEF000002))

        for sha_type in sha_types:
            log.info("\n########### Create CSR and Certificate for ket at location 0xEF000003 using openssl provider ###############")
            run("%s req -new --provider %s --provider default -key nxp:%s -out %s %s -%s" %(openssl_bin, provider, ref_ec_key_0xEF000002, output_csr, subject, sha_type))
